...
Item {
    ...
    property string gameVersion: "1.0.0"
    ...
    function initGame(resetScore = true) {
        ...
        if (resetScore) {
            score = 0;
            currentLevel = 1;
            lives = 3; // Reinicia as vidas quando reinicia o jogo
        }
    ...
    function eatDot() {
        ...
        if (pacmanY >= 0 && pacmanY < maze.length && pacmanX >= 0 && pacmanX < maze[pacmanY].length) {
            if (maze[pacmanY][pacmanX] === '.') {
                ...
                powerMode = true;
                powerModeTime = 50; // Com atualização a cada 33ms e decremento a cada 3 frames, 50 = ~5 segundos
                ghostBlinkCounter = 0;
